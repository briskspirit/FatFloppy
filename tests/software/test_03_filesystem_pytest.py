...
def _check_fat_mirror(fs: FATFilesystem):
    ...
    try:
        ...
        assert fat1_data == fat2_data, f"FAT tables (size {fat_size}) are not mirrored."
...
def test_02_initialization_invalid_boot_sig(tmp_path):
    ...
    disk_corrupt.set_geometry(FMT_144.geometry)
    fs_corrupt = FATFilesystem(disk_corrupt)
    assert not fs_corrupt.is_valid(), "Filesystem should be invalid with bad boot signature"
...
def test_05_read_file_in_root(fs_setup):
    ...
    fs.write_file(filename, filedata)
    _check_fat_mirror(fs)
    assert fs.read_file(filename) == filedata
...
def test_07_create_directory_in_root(fs_setup):
    ...
    assert filtered_dir_entries[1].starting_cluster == 0
    fat_val = _read_test_fat_entry(fs, entry.starting_cluster)
...
def test_08_create_file_in_subdir(fs_setup):
    ...
    assert entry.size == len(filedata)
    assert not entry.is_dir
    assert fs.read_file(filepath) == filedata
...
def test_12_write_large_file_multiple_clusters(fs_setup):
    ...
    assert c2 > 1 and c1 != c2
    c3 = _read_test_fat_entry(fs, c2)
    assert c3 > 1 and c2 != c3 and c1 != c3
    assert _read_test_fat_entry(fs, c3) == 0xFFF
    assert fs.read_file(filename) == filedata
...
def test_13_overwrite_file(fs_setup):
    ...
    assert size1 != size2
    assert fs.read_file(filename) == new_data
...
def test_15_nested_directories(fs_setup):
    ...
    assert not target_entries[0].is_dir
    fs.delete(path)
...
def test_21_init_with_different_geometry_720k(fs_setup):
    fs, disk = fs_setup
    disk.set_geometry(FMT_720.geometry)
...
def test_22_invalid_83_filenames(fs_setup):
    ...
    for name in invalid_names:
        ...
        if '.' not in name and ' ' not in name and '?' not in name and '*' not in name and name.upper() not in ["COM1", "PRN"] and not name.startswith('.'):
