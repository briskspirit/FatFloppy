...
def mock_fs_setup(request):
    ...
    struct.pack_into("<H", boot_sector_data, 0x0B, SECTOR_SIZE)
    ...
    def mock_read_sectors(start_c, start_h, start_s, num_sectors):
        ...
        try:
            ...
            return b"\x00" * num_sectors * SECTOR_SIZE
        for i in range(num_sectors):
            ...
            if current_lba in written_lba_data:
                ...
                data_chunk = (
                    fat_data[rel_offset : rel_offset + SECTOR_SIZE]
                    if 0 <= rel_offset < len(fat_data)
                    else b"\x00" * SECTOR_SIZE
                )
            elif ROOT_DIR_START_OFFSET <= current_offset < ROOT_DIR_END_OFFSET:
                rel_offset = current_offset - ROOT_DIR_START_OFFSET
                data_chunk = (
                    root_dir_data[rel_offset : rel_offset + SECTOR_SIZE]
                    if 0 <= rel_offset < len(root_dir_data)
                    else b"\x00" * SECTOR_SIZE
                )
            else:
                data_chunk = b"\x00" * SECTOR_SIZE
            data_chunk = data_chunk.ljust(SECTOR_SIZE, b"\x00")
            result.extend(data_chunk[:SECTOR_SIZE])
        return bytes(result)

    mock_disk.read_sector = MagicMock(
        side_effect=lambda c, h, s: mock_read_sectors(c, h, s, 1)
    )
    ...
    def mock_write_sectors(start_c, start_h, start_s, data):
        ...
        for i in range(num_sectors):
            ...
            chunk = data[chunk_start:chunk_end].ljust(SECTOR_SIZE, b"\x00")
            ...
            if FAT_START_OFFSET <= current_offset < FAT_END_OFFSET:
                ...
                if rel_offset + SECTOR_SIZE <= len(fat_data):
                    fat_data[rel_offset : rel_offset + SECTOR_SIZE] = chunk
            elif ROOT_DIR_START_OFFSET <= current_offset < ROOT_DIR_END_OFFSET:
                rel_offset = current_offset - ROOT_DIR_START_OFFSET
                if rel_offset + SECTOR_SIZE <= len(root_dir_data):
                    root_dir_data[rel_offset : rel_offset + SECTOR_SIZE] = chunk

    mock_disk.write_sector = MagicMock(
        side_effect=lambda c, h, s, data: mock_write_sectors(c, h, s, data)
    )
    mock_disk.write_sectors = MagicMock(side_effect=mock_write_sectors)
    mock_disk.flush = MagicMock()
    fs = FATFilesystem(mock_disk)
    assert fs.is_valid()
    assert fs.fat_cache is not None
    assert len(fs.fat_cache) == FAT_SIZE_BYTES
    assert fs.fat_cache[:3] == bytes([0xF9, 0xFF, 0xFF])
    mock_state = {
...
def test_03_delete_non_empty_directory(mock_fs_setup):
    ...
    with patch.object(fs, "_list_directory_by_cluster", side_effect=mock_list_cluster) as mock_list_func:
        ...
        assert mock_state["root_dir_data"][:SECTOR_SIZE][0] != 0xE5
...
def test_10_write_no_free_dir_entry_root(mock_fs_setup):
    fs, mock_state = mock_fs_setup
    dummy_entry = fs._create_directory_entry_bytes(
        "FILLER.   ", False, 0, 0, datetime.datetime.now()
    )
...
def test_11_write_no_free_dir_entry_subdir(mock_fs_setup):
    ...
    for lba in range(subdir_lba, subdir_lba_end + 1):
        ...
        mock_state["written_lba_data"][lba] = full_subdir_cluster_data[
            offset_in_full : offset_in_full + SECTOR_SIZE
        ]
    with patch.object(fs, "_find_free_cluster", return_value=None) as mock_find_free:
...
def test_14_fat12_odd_cluster_read_write(mock_fs_setup):
    ...
    fs.fat_cache = mock_state["fat_data"][:]
    value_to_write = 0xABC
    expected_word = 0xABC3
    fs._set_fat_entry_cached(cluster, value_to_write)
    written_word = struct.unpack_from("<H", fs.fat_cache, byte_offset)[0]
    assert written_word == expected_word
    assert fs.fat_dirty
    fs.fat_dirty = False
    fs.fat_cache = bytearray(FAT_SIZE_BYTES)
    struct.pack_into("<H", fs.fat_cache, byte_offset, expected_word)
