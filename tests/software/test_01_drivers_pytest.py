...
from unittest.mock import patch, mock_open # Import mock
...
def driver_setup(tmp_path):
    ...
    test_format_profile = FMT_144 # Keep reference to the full format object
    ...
    yield driver, test_img_path, bytes_per_sector, test_format # Yield the physical format object
    print(f"\n[Fixture Teardown] Driver test image {test_img_path} cleanup.")

def test_01_initialization_from_file(driver_setup):
    driver, test_img_path, _, geom = driver_setup # geom is PhysicalFormat here
    assert test_img_path.exists()
    assert len(driver.image_data) > 0
    # --- FIX: Access total_sectors and bytes_per_sector from PhysicalFormat ---
...
def test_05_write_sector_and_flush(driver_setup):
    ...
    # --- FIX: Use geom (PhysicalFormat) method ---
    offset = geom.chs_to_lba(cyl, head, sect) * bytes_per_sector
...
def test_06_read_beyond_image_size(driver_setup):
    driver, _, bytes_per_sector, geom = driver_setup # geom is PhysicalFormat
    invalid_cyl = geom.cylinders
    invalid_head, invalid_sect = 0, 1
    disk = Disk(driver)
    disk.set_geometry(geom)
    # Disk validation raises the error
    ...
    # Read last valid sector using the driver directly
...
def test_09_img_flush_io_error(driver_setup):
    driver, test_img_path, bytes_per_sector, _ = driver_setup
    # Make the driver dirty
    ...
    with patch("builtins.open", mock_open()) as mocked_file:
        ...
        with pytest.raises(IOError, match="Flush failed: Permission denied"):
            driver.flush()
    # Ensure dirty flag remains set after failed flush
    assert driver.dirty

def test_10_img_init_read_error(tmp_path):
    test_img_path = tmp_path / "unreadable.img"
    test_img_path.touch() # Create the file
