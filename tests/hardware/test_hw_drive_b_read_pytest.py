...
from .conftest import _run_gw_write, POPULATED_360K_IMG, GW_FORMAT_360K, expected_file_content
...
def prepared_controller(pytestconfig, expected_file_content):
    ...
    _run_gw_write(TARGET_DRIVE, POPULATED_360K_IMG, GW_FORMAT_360K, f"Populated {TARGET_FORMAT_KEY} for Read Tests", auto_mode)
...
class TestHardwareDriveBRead:
    def test_01_hw_B_list_root(self, prepared_controller):
        print(f"\nRunning: HW Read: List root directory of Drive B")
    ...
    def test_02_hw_B_list_dir1(self, prepared_controller):
        print(f"\nRunning: HW Read: List /DIR1 directory on Drive B")
        controller = prepared_controller
    ...
    def test_03_hw_B_list_subdir(self, prepared_controller):
        print(f"\nRunning: HW Read: List /DIR1/SUBDIR directory on Drive B")
        controller = prepared_controller
    ...
    def test_04_hw_B_read_root_file(self, prepared_controller):
        print(f"\nRunning: HW Read: Read /TEST.TXT content on Drive B")
        controller = prepared_controller
        filepath = "/TEST.TXT"
    ...
    def test_05_hw_B_read_dir1_files(self, prepared_controller):
        print(f"\nRunning: HW Read: Read files in /DIR1 on Drive B")
        controller = prepared_controller
    ...
    def test_06_hw_B_read_subdir_file(self, prepared_controller):
        print(f"\nRunning: HW Read: Read file in /DIR1/SUBDIR on Drive B")
        controller = prepared_controller
