...
The tests will:
1.  Use Greaseweazle to write a clean, formatted 1.44M disk image.
...
def prepared_controller(pytestconfig: Config) -> Generator[DiskController, None, None]:
    ...
    2.  Writes a blank, formatted 1.44M disk image to the floppy in Drive A
...
class TestHardwareDriveAWrite:
    ...
    def test_01_hw_A_create_write_read_delete_file(self, prepared_controller: DiskController) -> None:
    ...
    def test_02_hw_A_create_delete_dir(self, prepared_controller: DiskController) -> None:
    ...
    def test_03_hw_A_write_multicluster_file(self, prepared_controller: DiskController) -> None:
        ...
        # Content needs to be larger than 3 clusters (512 bytes each on 1.44M)
        content: bytes = b"ClusterData 1.44M " * 100  # Approx 1700 bytes
        ...
        try:
            ...
            found_entry = next((item for item in dir_listing if item.get('name', '').upper() == 'MULTI_A.BIN'), None)
