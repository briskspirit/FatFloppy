...
"""
PyInstaller spec file for FatFloppy Linux application.
Uses onedir mode for compatibility with AppImage packaging.
"""

import sys
from pathlib import Path

# Get project root (spec is in packaging/linux/, project root is ../..)
...
# Path to icon (PNG for Linux)
icon_path = str(project_root / "assets" / "icons" / "fatfloppy_icon.png")
...
a = Analysis(
    ...
    datas=[
        ...
        (str(Path(SPECPATH) / "49-greaseweazle.rules"), "doc"),
    ],
    hiddenimports=[
        # PyQt6 modules
        'PyQt6.QtCore',
...
# Remove unnecessary Qt libraries (Linux uses .so extension)
