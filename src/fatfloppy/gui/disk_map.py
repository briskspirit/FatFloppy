...
class DiskMapView:
    ...
    def draw_disk_map(
        ...
        if current_head >= geometry.heads:
            self._draw_message(
                f"Invalid head selected: {current_head}",
                app_font,
                text_color
            )
            return
    ...
    def _create_sector_tooltip(
        ...
        if layout_info and lba >= 0:
            ...
            if lba >= first_data_sector and allocation_unit_size > 0:
                allocation_unit = (
                    (lba - first_data_sector) // allocation_unit_size
                )
        ...
        if allocation_unit is not None:
            ...
            if (hasattr(self, '_selected_units') and
                    allocation_unit in self._selected_units):
                if (hasattr(self, '_selected_file_path') and
    ...
    def _draw_logical_layout_note(self, font: QFont, color: QColor) -> None:
        ...
        note_text = (
            "Logical disk layout (sectors numbered sequentially 0, 1, 2, ...)"
        )
    ...
    def _draw_sectors(
        ...
        for c in range(num_cylinders):
            ...
            for sector_idx in range(sectors_per_track):
                ...
                p1 = QPointF(
                    x0 + r_inner * math.cos(theta),
                    y0 + r_inner * math.sin(theta)
                )
                p2 = QPointF(
    ...
    def _draw_single_sector(
        ...
        try:
            ...
            physical_sector_id = track_format.logical_to_physical_sector(
                logical_index
            )
