...
class FileManager(QObject):
    ...
    def create_directory(self) -> None:
        ...
        if not ok or not dir_name:
            self.logger.debug(
                "Create Directory dialog cancelled or no name entered."
            )
            return
        ...
        if is_physical:
            ...
            def on_create_success(_):
                ...
                self.logger.info(
                    f"Successfully created directory: {full_path}"
                )
    ...
    def delete_selected_items(self) -> None:
        ...
        self.logger.info(
            f"Attempting to delete {len(paths_to_delete)} item(s): "
            f"{paths_to_delete}"
        )
        ...
        if is_physical:
            def delete_op(progress_callback=None):
                ...
                for idx, path in enumerate(paths_to_delete):
                    ...
                    try:
                        success = self.parent.controller.delete_item_recursive(
                            path
                        )
                        if not success:
    ...
    def import_multiple_paths(
        ...
        if (len(file_paths) == 1 and os.path.isfile(file_paths[0]) and
                not auto_name):
        ...
        if is_physical:
            def import_multiple_op(progress_callback=None):
                ...
                for idx, (local_path, dest_name) in enumerate(
                    file_paths_with_names
                ):
                    if progress_callback:
                        progress_callback(
                            idx,
                            total,
                            f"Importing {dest_name}..."
                        )

                    if os.path.isfile(local_path):
                        self._add_file_to_disk(
                            local_path,
                            dest_name,
                            target_path
                        )
                        ...
                        success = self.parent.controller.create_directory(
                            new_dir_path
                        )
                        if not success:
                            self.logger.error(
                                f"Failed to create directory {dest_name}"
                            )
                            continue
            ...
            def on_error(exception):
                self.error_occurred.emit(
                    "Error",
                    f"Failed to import items: {str(exception)}"
                )
                self.logger.exception("Error importing items.")

            item_word = (
                "item" if len(file_paths_with_names) == 1 else "items"
            )
            self.parent._run_threaded_operation(
                import_multiple_op,
            ...
            for local_path, dest_name in file_paths_with_names:
                if os.path.isfile(local_path):
                    ...
                    sub_paths = [
                        os.path.join(local_path, item) for item in items
                    ]
                    self.import_multiple_paths(
                        sub_paths,
                        new_dir_path,
            ...
            self.operation_complete.emit(
                f"Imported {len(file_paths_with_names)} {item_word} to "
                f"'{target_path}'"
            )
    ...
    def save_file(
        ...
        if is_physical:
            def save_op():
                if not self.parent.controller.write_file(
                    file_path,
                    content_bytes
                ):
            ...
            def on_save_success(saved_text):
                self.operation_complete.emit(
                    f"Saved changes to {os.path.basename(file_path)}"
                )
            ...
            def on_save_error(exception):
                ...
                self.logger.warning(
                    f"Failed to write file data for {file_path}"
                )
    ...
    def _add_file_to_disk(
        ...
        """
        self.logger.info(
            f"Adding local file '{file_path}' as '{dest_name}' to "
            f"'{dest_path}'."
        )
    ...
    def _extract_directory(
        ...
        try:
            self.logger.info(
                f"Extracting directory '{source_dir_path}' to "
                f"'{local_dir_path}'."
            )
            ...
            self.logger.info(
                f"Successfully extracted directory {source_dir_path}."
            )
        except Exception as e:
            self.error_occurred.emit(
                "Error",
                f"Failed to extract directory {source_dir_path}: {str(e)}"
            )
            self.logger.exception(
                f"Error extracting directory {source_dir_path}."
            )
    ...
    def _extract_file(self, source_path: str, local_path: str) -> None:
        ...
        try:
            self.logger.info(
                f"Extracting file '{source_path}' to '{local_path}'."
            )
            ...
            if file_data is not None:
                with open(local_path, 'wb') as f:
                    f.write(file_data)
                self.operation_complete.emit(
                    f"Extracted {os.path.basename(source_path)} to "
            ...
            self.error_occurred.emit(
                "Error",
                f"Failed to extract file {source_path}: {str(e)}"
            )
    ...
    def _import_directory_recursive(
        ...
        new_path = (
            f"{target_path}/{new_name}" if target_path != "/" else f"/{new_name}"
        )
