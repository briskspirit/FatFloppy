...
from PyQt6.QtWidgets import QFileDialog, QInputDialog, QMessageBox, QMainWindow
...
class FileManager(QObject):
    ...
    # Signals
    operation_complete = pyqtSignal(str)  # status message
    refresh_needed = pyqtSignal(str)  # path to refresh
    ...
    def add_file(self) -> None:
        """Opens a file dialog to select a local file and adds it to the current directory."""
        ...
        self.import_multiple_paths([file_path], self.parent.current_path, auto_name=False)
    ...
    def create_directory(self) -> None:
        ...
        dir_name, ok = QInputDialog.getText(
            self.parent, "Create New Directory",
        ...
        if is_physical:
            ...
            def on_create_success(_):
                ...
                self.operation_complete.emit(f"Created directory {dir_name} in {current_path}")
            ...
            try:
                ...
                if success:
                    ...
                    QMessageBox.warning(
                        self.parent, "Warning",
                        f"Failed to create directory {dir_name}. It might already exist or name is invalid."
                    )
                    self.logger.warning(f"Failed to create directory {full_path}.")
            except Exception as e:
                self.error_occurred.emit("Error", f"Failed to create directory: {str(e)}")
    ...
    def delete_selected_items(self) -> None:
        ...
        reply = QMessageBox.question(
            self.parent, "Confirm Deletion", msg,
            ...
            def on_delete_success(failed_paths):
                ...
                if not failed_paths:
                    ...
                    self.operation_complete.emit(
                        f"Deleted {len(paths_to_delete) - len(failed_paths)} item(s), "
            ...
            if node.is_dir:
                ...
                if is_physical:
                    ...
                    self.parent._run_threaded_operation(
                        ...
                        on_success=lambda _: self.operation_complete.emit("Extraction complete."),
                        cancelable=False
                    )
                else:
                ...
                save_path, _ = QFileDialog.getSaveFileName(self.parent, "Save File", node.name)
                if not save_path:
                    return
    ...
    def import_multiple_paths(
        ...
        Args:
            ...
            auto_name: If True, generates unique names automatically. If False, prompts for name.
        """
        if not file_paths:
            return
        ...
        if len(file_paths) == 1 and os.path.isfile(file_paths[0]) and not auto_name:
            ...
            existing_names = [
                item['name'].upper() for item in self.parent.controller.list_directory(target_path)
            ]
        ...
        if is_physical:
            def import_multiple_op(progress_callback=None):
                ...
                for idx, (local_path, dest_name) in enumerate(file_paths_with_names):
                    ...
                    if os.path.isfile(local_path):
                        ...
                        new_dir_path = (
                            f"{target_path}/{dest_name}" if target_path != "/"
                            else f"/{dest_name}"
                        )
            ...
            def on_success(count):
                ...
                self.operation_complete.emit(f"Imported {count} {item_word} to '{target_path}'")

            def on_error(exception):
                self.error_occurred.emit("Error", f"Failed to import items: {str(exception)}")
            ...
            self.refresh_needed.emit(target_path)
            item_word = "item" if len(file_paths_with_names) == 1 else "items"
            self.operation_complete.emit(f"Imported {len(file_paths_with_names)} {item_word} to '{target_path}'")
    ...
    def read_file_threaded(self, path: str, on_success: Callable) -> None:
        ...
        def success_handler(content):
            if content is not None:
                ...
                QMessageBox.warning(self.parent, "Warning", f"Could not read file: {path}")
    ...
    def save_file(
        ...
        if is_physical:
            ...
            def on_save_success(saved_text):
                self.operation_complete.emit(f"Saved changes to {os.path.basename(file_path)}")
                on_success(saved_text)

            def on_save_error(exception):
                QMessageBox.warning(
                    self.parent, "Save Failed",
    ...
    def _extract_directory(
        ...
        try:
            ...
            self.error_occurred.emit("Error", f"Failed to extract directory {source_dir_path}: {str(e)}")
        ...
        try:
            ...
            if file_data is not None:
                with open(local_path, 'wb') as f:
                    f.write(file_data)
                self.operation_complete.emit(
                    f"Extracted {os.path.basename(source_path)} to {os.path.basename(local_path)}"
                )
            ...
            self.error_occurred.emit("Error", f"Failed to extract file {source_path}: {str(e)}")
