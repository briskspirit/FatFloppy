...
class DiskManager(QObject):
    ...
    def open_physical_floppy(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Opening physical floppy: Drive {drive_letter}, "
                f"Size {drive_size}, Format {format_info}"
            )
        ...
        if self.parent.controller.filesystem:
            ...
            if "Filesystem Type" in fs_info_dict:
                fs_info_lines.append(
                    f"Filesystem Type: {fs_info_dict.pop('Filesystem Type')}"
                )
            if "Volume Label" in fs_info_dict:
                fs_info_lines.append(
                    f"Volume Label: {fs_info_dict.pop('Volume Label')}"
                )
            ...
            if space_info:
                ...
                space_str = (
                    f"Free Space: {free_kb:.1f} KB / {total_kb:.1f} KB "
                    f"({percent_free:.1f}%)"
                )
            else:
                space_str = "Free Space: N/A"
                self.logger.warning(
                    "Could not retrieve filesystem free space information."
                )
    ...
    def update_space_info(self) -> None:
        ...
        try:
            ...
            if self.parent.controller.filesystem:
                if self.parent.controller.filesystem.allocation_unit_size > 0:
                    allocation_unit_size_bytes = (
                        self.parent.controller.filesystem.allocation_unit_size
                    )
                else:
                    self.logger.warning(
                        "Filesystem reported zero allocation unit size. "
                        "Using default 512 bytes."
                    )
                ...
                if space_info:
                    ...
                    if allocation_unit_size_bytes > 0:
                        ...
                        self.logger.warning(
                            "Cannot calculate space in units "
                            "(allocation unit size is zero)."
                        )
                        self.free_space = 0
                        self.total_space = 0
                else:
                ...
                self.logger.warning(
                    "No filesystem detected, cannot calculate accurate "
                    "free/total space."
                )
                self.busy_units = []
                self.free_space = 0
            ...
            self.logger.debug(
                f"Busy units: {len(self.busy_units)}, Free units: "
                f"{self.free_space}, Total units: {self.total_space}"
            )
        ...
        if is_image:
            ...
            if format_info and not format_info.get("profile_name"):
                format_text += (
                    ...
                    f"{format_info.get('heads')}x"
                    f"{format_info.get('sectors_per_track')})"
                )
            self.status_message.emit(
    ...
    def _get_format_profile(self, format_info: dict[str, Any]) -> Optional[Any]:
        ...
        if "profile_name" in format_info:
            ...
            if original_profile:
                self.logger.debug(
                    f"Retrieved predefined format profile: {profile_name}"
                )
            ...
            controller = self.parent.controller or DiskController()
            self.logger.debug(
                f"Creating custom format profile with info: {format_info}"
            )
