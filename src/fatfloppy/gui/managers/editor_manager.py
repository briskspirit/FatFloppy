...
class EditorManager(QObject):
    ...
    # Signals
    content_loaded = pyqtSignal(str, str, str)  # file_path, content, viewer_type ('text' or 'hex')
    modified_changed = pyqtSignal(bool)  # is_modified
    save_complete = pyqtSignal(str)  # saved_text
    refresh_needed = pyqtSignal(str)  # current_path
    ...
    def discard_changes(self) -> None:
        """Discards any unsaved changes in the text editor and reverts to original content."""
        if self.original_text_content is not None and self.text_editor_modified:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                self.parent.statusBar().showMessage(
                    f"Changes to {os.path.basename(self.current_file_path)} discarded."
                )
    ...
    def save_file(self) -> bool:
        ...
        if not self.current_file_path or not self.text_editor_modified:
            if not self.text_editor_modified:
                ...
                QMessageBox.warning(self.parent, "Warning", "No file context for saving.")
        ...
        try:
            ...
            if fs_type == "CP/M":
                content_bytes = bytes([
                    b & 0x7F for b in normalized_text.encode('ascii', errors='replace')
                ])
            else:
            ...
            # Use FileManager's save_file method
    ...
    def view_file_content(self) -> None:
        ...
        node = item.node
        if node.is_dir:
            QMessageBox.information(self.parent, "Info", "Cannot view directory contents.")
            return
